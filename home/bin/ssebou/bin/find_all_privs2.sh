...
# this is to printout all the privileges of a user
# sqlplus utility is used
# Adopted from original sql script mentioned by url below to be just bash script

[ -z "$1" ] && { >&2 echo "Usage: system/system@host/dbsid username" ; exit 1; }
[ -z "$2" ] && { >&2 echo "Usage: system/system@host/dbsid username" ; exit 1; }
command sqlplus --version >/dev/null 2>&1 || { >&2 echo "No sqlplus available here" ; exit 1; }

user_to_find=$2
output_method=s
examine_tables="N"
# 2 vars below out of use , just given some value
file_name="priv.lst"
output_dir="/tmp"
sqlplus -s  $1 <<EOS
-- -----------------------------------------------------------------------------
