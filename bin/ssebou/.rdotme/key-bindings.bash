# Key bindings
# ------------
export FZFTRM="gnome-terminal --geometry=130x32"

fzf_view_less () {
  local FIN="$1"
  local RABSP
  cat $FIN | while read RABSP
  do
      [ ! -z  "$RABSP" ] && ${FZFTRM}  -e "less \"$RABSP\" "
  done
}

export -f  fzf_view_less

__fzf_select__() {
  local tf=$(mktemp "/tmp/fzf_select.XXXX")
  local cmd="${FZF_CTRL_T_COMMAND:-"command find -L . \\( -path '*/\\.*' -o -fstype 'dev' -o -fstype 'proc' \\) -prune \
    -o -type f -print \
    -o -type d -print \
    -o -type l -print 2> /dev/null | sed 1d | cut -b3-"}"
  eval "$cmd" | 
  fzf -m $FZF_CTRL_T_OPTS \
   --color=dark,bg+:160 \
   --header="view:f3" \
   --bind "f3:execute-multi( printf \"%s\n\" {} >$tf ; fzf_view_less $tf ;rm $tf )"\
   --bind "f4:execute-multi( bash )"\
  |
...
__fzf_history__() (
  ...
  line=$(
    cat ${HISTKEEP3} | grep -a -v 'PROMPT_COMMAND=.*kill' | grep -a -v -f ${HISTKEEP3IGNORE} |
    eval "$(__fzfcmd) +s --tac +m -n2..,.. --tiebreak=index --toggle-sort=ctrl-r --bind=alt-j:jump-accept $FZF_CTRL_R_OPTS" |
    command grep -a '^.*') &&
    if [[ $- =~ H ]]; then
      # when histexpand flag is present
      sed 's/^ *\([0-9]*\)\** .*/!\1/' <<< "$line"
    else
      sed 's/^ *\([0-9]*\)\** *//' <<< "$line"
    fi
